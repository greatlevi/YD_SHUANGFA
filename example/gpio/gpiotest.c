...
static void USER_FUNC test_gpio_start()
{
	...
	//hfgpio_configure_fpin(HFGPIO_F_NREADY,HFM_IO_TYPE_INPUT);
	while(1)
	{
		if(!(hfgpio_fconfigure_get(HFGPIO_F_NREADY)&HFM_IO_TYPE_INPUT))
		{
			u_printf("NREADY switch to input mode!\n");
			hfgpio_configure_fpin(HFGPIO_F_NREADY,HFM_IO_TYPE_INPUT);
		}
		msleep(1000);
		if(hfgpio_fpin_is_high(HFGPIO_F_NREADY)>=1)
		{
			u_printf("NREADY is high!\n");
		}
		else
		{
			u_printf("NREADY is low!\n");
		}
